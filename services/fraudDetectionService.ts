...
const generateDeviceFingerprint = async (): Promise<string> => {
  try {
    ...
    console.error('❌ [FRAUD] Error generating device fingerprint:', error);
...
const extractPostId = (url: string): string | null => {
  try {
    ...
    console.error('❌ [FRAUD] Error extracting post ID:', error);
    return null;
  }
};
...
const getSubmissionHistory = async (): Promise<SubmissionRecord[]> => {
  try {
    ...
    console.error('❌ [FRAUD] Error getting submission history:', error);
    return [];
  }
};
...
const addSubmissionToHistory = async (url: string, postId: string): Promise<void> => {
  try {
    ...
    console.error('❌ [FRAUD] Error adding submission to history:', error);
  }
};
...
const isUserBlocked = async (): Promise<{ blocked: boolean; until?: number; reason?: string }> => {
  try {
    ...
    console.error('❌ [FRAUD] Error checking block status:', error);
    return { blocked: false };
  }
};
...
const blockUser = async (durationMs: number, reason: string): Promise<void> => {
  try {
    ...
    console.error('❌ [FRAUD] Error blocking user:', error);
  }
};
...
export const checkDuplicateUrl = async (url: string): Promise<DuplicateCheckResult> => {
  try {
    ...
    console.error('❌ [FRAUD] Error checking duplicate:', error);
...
export const checkRateLimit = async (): Promise<RateLimitStatus> => {
  try {
    ...
    console.error('❌ [FRAUD] Error checking rate limit:', error);
...
export const recordSubmission = async (url: string): Promise<void> => {
  try {
    const postId = extractPostId(url);
    if (!postId) {
      console.warn('⚠️ [FRAUD] Could not extract post ID for recording');
      return;
    }
    ...
    console.error('❌ [FRAUD] Error recording submission:', error);
  }
};
...
export const clearSubmissionHistory = async (): Promise<void> => {
  try {
    ...
    console.error('❌ [FRAUD] Error clearing history:', error);
  }
};
...
export const getFraudStats = async (): Promise<{
  ...
  try {
    ...
    console.error('❌ [FRAUD] Error getting fraud stats:', error);
    return {
      totalSubmissions: 0,
      submissionsToday: 0,
