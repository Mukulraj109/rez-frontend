...
class MenuApi {
  ...
  async getStoreMenu(storeId: string): Promise<ApiResponse<StoreMenu>> {
    try {
      console.log('🍽️ [MENU API] Fetching menu for store:', storeId);

      const response = await apiClient.get(`${this.baseUrl}/store/${storeId}`);

      if (response.success && response.data) {
        console.log('✅ [MENU API] Menu loaded:', response.data.categories?.length, 'categories');
        return response;
      }

      console.warn('⚠️ [MENU API] No menu data received');
      return {
        success: false,
  ...
  async getMenuItem(menuItemId: string): Promise<ApiResponse<MenuItem>> {
    try {
      console.log('🍔 [MENU API] Fetching menu item:', menuItemId);

      const response = await apiClient.get(`${this.baseUrl}/items/${menuItemId}`);

      if (response.success && response.data) {
        console.log('✅ [MENU API] Menu item loaded:', response.data.name);
        return response;
      }
  ...
  async createPreOrder(preOrderData: PreOrderRequest): Promise<ApiResponse<PreOrder>> {
    try {
      console.log('🛒 [MENU API] Creating pre-order for store:', preOrderData.storeId);

      const response = await apiClient.post(`${this.baseUrl}/pre-orders`, preOrderData);

      if (response.success && response.data) {
        console.log('✅ [MENU API] Pre-order created:', response.data.orderNumber);
        return response;
      }
      ...
      console.error('❌ [MENU API] Error creating pre-order:', error);
      return {
        success: false,
  ...
  async getUserPreOrders(): Promise<ApiResponse<PreOrder[]>> {
    try {
      console.log('📋 [MENU API] Fetching user pre-orders');

      const response = await apiClient.get(`${this.baseUrl}/pre-orders/user`);

      if (response.success && response.data) {
        console.log('✅ [MENU API] Loaded', response.data.length, 'pre-orders');
        return response;
      }
      ...
      console.error('❌ [MENU API] Error fetching pre-orders:', error);
      return {
        success: false,
  ...
  async getPreOrder(preOrderId: string): Promise<ApiResponse<PreOrder>> {
    try {
      console.log('🔍 [MENU API] Fetching pre-order:', preOrderId);

      const response = await apiClient.get(`${this.baseUrl}/pre-orders/${preOrderId}`);

      if (response.success && response.data) {
        console.log('✅ [MENU API] Pre-order loaded:', response.data.orderNumber);
        return response;
      }
      ...
      console.error('❌ [MENU API] Error fetching pre-order:', error);
      return {
        success: false,
  ...
  async cancelPreOrder(preOrderId: string): Promise<ApiResponse<PreOrder>> {
    try {
      console.log('❌ [MENU API] Cancelling pre-order:', preOrderId);

      const response = await apiClient.put(`${this.baseUrl}/pre-orders/${preOrderId}/cancel`);

      if (response.success && response.data) {
        console.log('✅ [MENU API] Pre-order cancelled successfully');
        return response;
      }
      ...
      console.error('❌ [MENU API] Error cancelling pre-order:', error);
      return {
        success: false,
  ...
  async searchMenuItems(query: string, storeId?: string): Promise<ApiResponse<MenuItem[]>> {
    try {
      console.log('🔍 [MENU API] Searching menu items:', query);
      ...
      if (response.success && response.data) {
        console.log('✅ [MENU API] Found', response.data.length, 'menu items');
        return response;
      }
      ...
      console.error('❌ [MENU API] Error searching menu items:', error);
      return {
        success: false,
