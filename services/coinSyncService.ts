...
class CoinSyncService {
  ...
  async getWalletBalance(): Promise<number> {
    try {
      console.log('🔄 [COIN SYNC] Fetching wallet balance (source of truth)...');
      ...
      console.error('❌ [COIN SYNC] Error fetching wallet balance:', error);
      throw error;
    }
  }
  ...
  async getPointsBalance(): Promise<PointsBalance | null> {
    try {
      console.log('🔄 [COIN SYNC] Fetching points balance for verification...');

      const response = await pointsApi.getBalance();

      if (response.success && response.data) {
        console.log(`✅ [COIN SYNC] Points balance: ${response.data.total}`);
        return response.data;
      }

      return null;
    } catch (error) {
      console.warn('⚠️ [COIN SYNC] Could not fetch points balance:', error);
      return null;
    }
  }
  ...
  async syncGamificationReward(
    ...
    try {
      console.log(`🎮 [COIN SYNC] Syncing gamification reward: ${amount} coins from ${source}`);
      ...
      console.log(`✅ [COIN SYNC] Points awarded: ${amount}`);

      // Step 2: Verify wallet balance was updated
      const newWalletBalance = await this.getWalletBalance();

      console.log(`✅ [COIN SYNC] Reward synced successfully. New wallet balance: ${newWalletBalance}`);
      ...
      console.error('❌ [COIN SYNC] Error syncing gamification reward:', error);
      return {
        success: false,
        coinsAdded: 0,
        newWalletBalance: 0,
  ...
  async spendCoins(
    ...
    try {
      console.log(`💸 [COIN SYNC] Spending ${amount} coins for: ${purpose}`);
      ...
      console.log(`✅ [COIN SYNC] Points spent: ${amount}`);

      // Step 2: Verify wallet balance was updated
      const newWalletBalance = await this.getWalletBalance();

      console.log(`✅ [COIN SYNC] Coins spent successfully. New wallet balance: ${newWalletBalance}`);
      ...
      console.error('❌ [COIN SYNC] Error spending coins:', error);
      return {
        success: false,
        coinsAdded: 0,
        newWalletBalance: 0,
  ...
  async checkSync(): Promise<CoinSyncResult> {
    try {
      console.log('🔍 [COIN SYNC] Checking wallet and points sync status...');
      ...
      if (synced) {
        console.log(`✅ [COIN SYNC] Wallet and points are in sync: ${walletBalance}`);
      } else {
        console.warn(
      ...
      console.error('❌ [COIN SYNC] Error checking sync:', error);
      return {
        success: false,
        walletBalance: 0,
        pointsBalance: 0,
  ...
  async forceSyncPointsToWallet(): Promise<CoinSyncResult> {
    if (this.syncInProgress) {
      console.warn('⚠️ [COIN SYNC] Sync already in progress, skipping...');
      return {
        success: false,
        walletBalance: 0,
        pointsBalance: 0,
    ...
    try {
      ...
      console.log('🔄 [COIN SYNC] Force syncing points to wallet...');

      const syncStatus = await this.checkSync();

      if (syncStatus.synced) {
        console.log('✅ [COIN SYNC] Already in sync, no action needed');
        return syncStatus;
      }
      ...
      console.error('❌ [COIN SYNC] Error force syncing:', error);
      return {
        success: false,
        walletBalance: 0,
        pointsBalance: 0,
  ...
  async handleGameReward(
    ...
    console.log(`🎮 [COIN SYNC] Handling ${gameType} reward: ${coinsWon} coins`);
  ...
  async handleChallengeReward(
    ...
    console.log(`🏆 [COIN SYNC] Handling challenge reward: ${coinsReward} coins for ${challengeName}`);
  ...
  async handleAchievementReward(
    ...
    console.log(`🎖️ [COIN SYNC] Handling achievement reward: ${coinsReward} coins for ${achievementName}`);
  ...
  async handleDailyLoginReward(
    ...
    console.log(`📅 [COIN SYNC] Handling daily login reward: ${coinsReward} coins (streak: ${streak})`);
  ...
  private updateLastSyncTime(): void {
    try {
      ...
      console.warn('⚠️ [COIN SYNC] Could not update last sync time:', error);
    }
  }
}
