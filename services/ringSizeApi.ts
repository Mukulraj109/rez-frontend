...
class RingSizeApiService {
  ...
  async saveRingSize(
    ...
    try {
      ...
      try {
        ...
        console.warn('💍 [RING SIZE API] Local storage save failed:', storageError);
      }
      ...
      try {
        ...
        console.warn('💍 [RING SIZE API] Backend save failed:', apiError);
      ...
      console.error('❌ [RING SIZE API] Save ring size failed:', error);
      return {
        success: false,
  ...
  async getRingSize(): Promise<ApiResponse<RingSizeData>> {
    try {
      ...
      try {
        ...
        console.warn('💍 [RING SIZE API] Backend fetch failed, trying local storage');
      }
      ...
      console.error('❌ [RING SIZE API] Get ring size failed:', error);
      return {
        success: false,
  ...
  async deleteRingSize(): Promise<ApiResponse<void>> {
    try {
      ...
      try {
        ...
        console.warn('💍 [RING SIZE API] Backend delete failed:', apiError);
        return {
          success: true,
      ...
      console.error('❌ [RING SIZE API] Delete ring size failed:', error);
      return {
        success: false,
  ...
  async syncToBackend(): Promise<ApiResponse<RingSizeData>> {
    try {
      ...
      console.error('❌ [RING SIZE API] Sync failed:', error);
      return {
        success: false,
