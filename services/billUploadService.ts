...
class BillUploadService {
  ...
  async uploadBillWithProgress(
    ...
    try {
      console.log('📤 [BILL UPLOAD] Starting upload with progress tracking...');
      ...
      return await new Promise<ApiResponse<Bill>>((resolve, reject) => {
        ...
        xhr.upload.addEventListener('progress', (event) => {
          if (event.lengthComputable && onProgress) {
            ...
            console.log(`📊 [BILL UPLOAD] Progress: ${progress.percentage}% (${this.formatSpeed(speed)})`);
            onProgress(progress);
          }
        });
        ...
        xhr.addEventListener('load', () => {
          ...
          if (xhr.status >= 200 && xhr.status < 300) {
            try {
              ...
              console.log('✅ [BILL UPLOAD] Upload completed successfully');
              resolve({
                success: true,
              ...
              console.error('❌ [BILL UPLOAD] Failed to parse response:', err);
              reject(this.createUploadError(UploadErrorCode.SERVER_ERROR, 'Invalid server response'));
            }
          } else {
            try {
              const errorResponse = JSON.parse(xhr.responseText);
              console.error('❌ [BILL UPLOAD] Upload failed:', errorResponse);
              resolve({
                success: false,
        ...
        xhr.addEventListener('error', () => {
          this.activeUploads.delete(uploadId);
          this.uploadSpeeds.delete(uploadId);
          console.error('❌ [BILL UPLOAD] Network error occurred');
        ...
        xhr.addEventListener('timeout', () => {
          this.activeUploads.delete(uploadId);
          this.uploadSpeeds.delete(uploadId);
          console.error('❌ [BILL UPLOAD] Upload timeout');
        ...
        xhr.addEventListener('abort', () => {
          this.activeUploads.delete(uploadId);
          this.uploadSpeeds.delete(uploadId);
          console.log('⚠️ [BILL UPLOAD] Upload cancelled');
        ...
        // Send request
        console.log('🚀 [BILL UPLOAD] Sending request to:', url);
        xhr.send(formData);
      });
    } catch (error) {
      this.activeUploads.delete(uploadId);
  ...
  async uploadBillWithRetry(
    ...
    console.log('🔄 [BILL UPLOAD] Starting upload with retry (max attempts:', config.maxAttempts, ')');

    for (let attempt = 1; attempt <= config.maxAttempts; attempt++) {
      try {
        console.log(`📤 [BILL UPLOAD] Attempt ${attempt}/${config.maxAttempts}`);
        ...
        if (result.success && result.data) {
          ...
          console.log('✅ [BILL UPLOAD] Upload successful after', attempt, 'attempt(s)');
        ...
        console.error(`❌ [BILL UPLOAD] Attempt ${attempt} failed:`, lastError.message);
      }

      // Check if error is retryable
      if (lastError && !config.retryableErrors.includes(lastError.code)) {
        console.log('⚠️ [BILL UPLOAD] Error is not retryable:', lastError.code);
        break;
      }
      ...
      if (attempt < config.maxAttempts) {
        ...
        console.log(`⏳ [BILL UPLOAD] Waiting ${delay}ms before retry...`);
    ...
    // All attempts failed
    const duration = Date.now() - startTime;
    console.error('❌ [BILL UPLOAD] All retry attempts failed');
  ...
  cancelUpload(uploadId: string): boolean {
    ...
    if (xhr) {
      ...
      this.activeUploads.delete(uploadId);
      this.uploadSpeeds.delete(uploadId);
      console.log('🛑 [BILL UPLOAD] Upload cancelled:', uploadId);
      return true;
    }
    return false;
  }
  ...
  private async createFormData(data: BillUploadData): Promise<FormData> {
    ...
    if (!validation.isValid) {
      console.error('❌ [BILL UPLOAD] Invalid file extension:', validation.error);
    ...
    if (Platform.OS === 'web') {
      try {
        console.log('🌐 [BILL UPLOAD] Fetching image blob for web upload...');
        ...
        console.log('✅ [BILL UPLOAD] Image blob fetched successfully:', blob.size, 'bytes');
        formData.append('billImage', blob, filename);
      } catch (error) {
        console.error('❌ [BILL UPLOAD] Failed to fetch image blob:', error);
        throw this.createUploadError(
  ...
  async getBillById(billId: string): Promise<ApiResponse<Bill>> {
    try {
      console.log('📋 [BILL DETAIL] Fetching bill:', billId);
      const response = await apiClient.get<Bill>(`/bills/${billId}`);

      if (response.success && response.data) {
        console.log('✅ [BILL DETAIL] Bill fetched successfully:', {
  ...
  async resubmitBill(billId: string, newPhoto: string): Promise<ApiResponse<Bill>> {
    try {
      console.log('🔄 [BILL RESUBMIT] Resubmitting bill:', billId);
      ...
      const validation = this.validateFileExtension(filename);
      if (!validation.isValid) {
        console.error('❌ [BILL RESUBMIT] Invalid file extension:', validation.error);
        return {
          success: false,
      ...
      if (Platform.OS === 'web') {
        try {
          console.log('🌐 [BILL RESUBMIT] Fetching image blob for web upload...');
          ...
          console.log('✅ [BILL RESUBMIT] Image blob fetched successfully:', blob.size, 'bytes');
          formData.append('billImage', blob, filename);
        } catch (error) {
          console.error('❌ [BILL RESUBMIT] Failed to fetch image blob:', error);
          return {
            success: false,
      ...
      if (response.success && response.data) {
        console.log('✅ [BILL RESUBMIT] Bill resubmitted successfully:', {
          billId: response.data._id,
          status: response.data.verificationStatus,
        });
        return response;
  ...
  async getBillStatistics(): Promise<ApiResponse<{
    ...
    try {
      console.log('📊 [BILL STATS] Fetching bill statistics...');
      ...
      if (response.success && response.data) {
        console.log('✅ [BILL STATS] Statistics fetched successfully:', {
