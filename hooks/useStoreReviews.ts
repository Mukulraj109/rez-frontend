...
export function useStoreReviews(
  ...
  const fetchReviews = useCallback(async (page: number = 1, append: boolean = false) => {
    ...
    try {
      ...
      console.log('🔍 [useStoreReviews] Raw API Response:', JSON.stringify(response, null, 2));

      if (response.success && response.data) {
        const { reviews: fetchedReviews, summary, pagination } = response.data;
        
        console.log('📦 [useStoreReviews] Fetched Reviews Count:', fetchedReviews?.length || 0);
        console.log('📦 [useStoreReviews] Raw Fetched Reviews:', JSON.stringify(fetchedReviews, null, 2));
        console.log('📊 [useStoreReviews] Summary:', JSON.stringify(summary, null, 2));
        console.log('📄 [useStoreReviews] Pagination:', JSON.stringify(pagination, null, 2));
        ...
        const transformedReviews: Review[] = fetchedReviews.map((review: any, index: number) => {
          console.log(`🔄 [useStoreReviews] Transforming Review ${index + 1}:`, JSON.stringify(review, null, 2));
          ...
          if (review.user?.name) {
            ...
            console.log(`  ✅ [useStoreReviews] Review ${index + 1} - Using user.name:`, userName);
            ...
            console.log(`  ✅ [useStoreReviews] Review ${index + 1} - Combined from profile:`, {
              firstName,
              lastName,
              userName
            });
          } else {
            console.log(`  ⚠️ [useStoreReviews] Review ${index + 1} - No user data found, using Anonymous`);
            console.log(`  📋 [useStoreReviews] Review ${index + 1} - Full user object:`, JSON.stringify(review.user, null, 2));
          }
          ...
          console.log(`  ✨ [useStoreReviews] Review ${index + 1} - Transformed:`, JSON.stringify(transformed, null, 2));
          
          return transformed;
        });
        
        console.log('✅ [useStoreReviews] All Transformed Reviews:', JSON.stringify(transformedReviews, null, 2));
