...
export const usePriceTracking = ({
  ...
  const loadPriceHistory = useCallback(
    async (options?: { limit?: number; days?: number }) => {
      ...
      try {
        ...
        console.log('📊 [usePriceTracking] Loading price history');
        ...
        if (response.success) {
          ...
          console.log('✅ [usePriceTracking] History loaded:', response.data.count);
        }
      } catch (err: any) {
        console.error('❌ [usePriceTracking] Load history error:', err);
  ...
  const loadPriceStats = useCallback(
    async (days: number = 30) => {
      ...
      try {
        ...
        console.log('📈 [usePriceTracking] Loading price stats');
        ...
        if (response.success) {
          ...
          console.log('✅ [usePriceTracking] Stats loaded');
        }
      } catch (err: any) {
        console.error('❌ [usePriceTracking] Load stats error:', err);
  ...
  const checkAlert = useCallback(async () => {
    ...
    try {
      console.log('🔍 [usePriceTracking] Checking alert status');

      const response = await priceTrackingApi.checkAlert(productId, variantId);

      if (response.success) {
        setHasActiveAlert(response.data.hasActiveAlert);
        console.log('✅ [usePriceTracking] Has active alert:', response.data.hasActiveAlert);
      }
    } catch (err: any) {
      console.error('❌ [usePriceTracking] Check alert error:', err);
    }
  }, [productId, variantId]);
  ...
  const createAlert = useCallback(
    async (data: Omit<CreatePriceAlertRequest, 'productId' | 'variantId'>) => {
      ...
      try {
        ...
        console.log('🔔 [usePriceTracking] Creating price alert');
        ...
        if (response.success) {
          ...
          console.log('✅ [usePriceTracking] Alert created successfully');
        ...
        console.error('❌ [usePriceTracking] Create alert error:', err);
  ...
  const cancelAlert = useCallback(async (alertId: string) => {
    try {
      ...
      console.log('🔕 [usePriceTracking] Cancelling alert');
      ...
      if (response.success) {
        ...
        console.log('✅ [usePriceTracking] Alert cancelled');
      }
    } catch (err: any) {
      console.error('❌ [usePriceTracking] Cancel alert error:', err);
  ...
  const loadAlerts = useCallback(async (params?: { page?: number; status?: string }) => {
    try {
      ...
      console.log('📋 [usePriceTracking] Loading alerts');

      const response = await priceTrackingApi.getMyAlerts(params);

      if (response.success) {
        setAlerts(response.data.alerts);
        console.log('✅ [usePriceTracking] Alerts loaded:', response.data.alerts.length);
      }
    } catch (err: any) {
      console.error('❌ [usePriceTracking] Load alerts error:', err);
