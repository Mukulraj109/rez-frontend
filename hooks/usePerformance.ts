...
export const usePerformance = (options: UsePerformanceOptions = {}): UsePerformanceReturn => {
  ...
  const startMeasure = useCallback(
    (label: string) => {
      ...
      if (enableLogging) {
        console.log(`⏱️ [Performance] Started: ${componentName} - ${label}`);
      }
    },
  ...
  const resetMetrics = useCallback(() => {
    ...
    console.log(`🔄 [Performance] ${componentName} metrics reset`);
  }, [componentName]);
  ...
  useEffect(() => {
    return () => {
      if (enableLogging && renderCountRef.current > 0) {
        console.group(`📊 [Performance] ${componentName} Final Metrics`);
