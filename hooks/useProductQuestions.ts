...
export const useProductQuestions = ({
  ...
  const loadQuestions = useCallback(
    async (pageNum: number = 1, refresh: boolean = false) => {
      ...
      try {
        ...
        console.log(`📋 [useProductQuestions] Loading questions - Page: ${pageNum}, Sort: ${sortBy}`);
        ...
        if (response.success) {
          ...
          console.log('✅ [useProductQuestions] Questions loaded:', response.data.questions.length);
        } else {
          throw new Error('Failed to load questions');
        }
      } catch (err: any) {
        console.error('❌ [useProductQuestions] Error loading questions:', err);
  ...
  const askQuestion = useCallback(
    async (questionText: string) => {
      try {
        console.log('❓ [useProductQuestions] Asking question:', questionText);
        ...
        if (response.success) {
          console.log('✅ [useProductQuestions] Question posted successfully');
        ...
        console.error('❌ [useProductQuestions] Error asking question:', err);
        throw err;
      }
    },
    [productId, refresh]
  ...
  const answerQuestion = useCallback(
    async (questionId: string, answerText: string) => {
      try {
        console.log('💬 [useProductQuestions] Answering question:', questionId);
        ...
        if (response.success) {
          console.log('✅ [useProductQuestions] Answer posted successfully');
        ...
        console.error('❌ [useProductQuestions] Error answering question:', err);
        throw err;
      }
    },
    []
  ...
  const markAnswerHelpful = useCallback(async (questionId: string, answerId: string) => {
    try {
      console.log('👍 [useProductQuestions] Marking answer as helpful:', answerId);

      const response = await questionsApi.markAnswerHelpful(questionId, answerId);

      if (response.success) {
        console.log('✅ [useProductQuestions] Vote recorded');
      ...
      console.error('❌ [useProductQuestions] Error marking helpful:', err);
      throw err;
    }
  }, []);
