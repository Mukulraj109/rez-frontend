...
export const useModePersistence = (): UseModePersistenceReturn => {
      ...
      try {
        ...
        if (saved) {
          ...
          if (VALID_MODES.includes(parsedMode as ModeId)) {
            ...
            console.warn('🔄 [MODE] Invalid persisted mode, using default:', parsedMode);
        ...
        console.error('🔄 [MODE] Failed to load persisted mode:', error);
        setStoredMode(DEFAULT_MODE);
      } finally {
        setIsLoaded(true);
      }
  ...
  const saveMode = useCallback(async (mode: ModeId): Promise<void> => {
    ...
    if (!VALID_MODES.includes(mode)) {
      console.warn('🔄 [MODE] Attempted to save invalid mode:', mode);
      return;
    }
    ...
    saveTimeoutRef.current = setTimeout(async () => {
      try {
        ...
        console.error('🔄 [MODE] Failed to save mode:', error);
      }
    }, 100); // 100ms debounce
  }, []);
  ...
  const clearMode = useCallback(async (): Promise<void> => {
    try {
      ...
      console.error('🔄 [MODE] Failed to clear mode:', error);
    }
  }, []);
