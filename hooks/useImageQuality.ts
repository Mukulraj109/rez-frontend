...
export function useImageQuality(options: QualityCheckOptions = {}) {
  ...
  const getCachedResult = useCallback(
    (uri: string): ImageQualityResult | null => {
      ...
      console.log('📦 [IMAGE QUALITY] Using cached result for:', uri);
  ...
  const cacheResult = useCallback((uri: string, result: ImageQualityResult) => {
    ...
    console.log('💾 [IMAGE QUALITY] Result cached for:', uri);
  }, []);
  ...
  const clearCache = useCallback(() => {
    ...
    console.log('🗑️ [IMAGE QUALITY] Cache cleared');
  }, []);
  ...
  const checkQuality = useCallback(
    async (uri: string): Promise<ImageQualityResult> => {
      ...
      try {
        console.log('🔍 [IMAGE QUALITY] Checking quality for:', uri);

        // Get image dimensions
        const { width, height } = await getImageDimensions(uri);
        console.log(`📐 [IMAGE QUALITY] Dimensions: ${width}x${height}`);

        // Get file size
        const fileSize = await getFileSize(uri);
        console.log(`📦 [IMAGE QUALITY] File size: ${(fileSize / 1024).toFixed(2)} KB`);
        ...
        console.log(`✅ [IMAGE QUALITY] Check complete - Score: ${score}/100, Valid: ${isValid}`);
        ...
        console.error('❌ [IMAGE QUALITY] Error:', errorMessage);
