...
export function useBillUpload(retryConfig?: Partial<RetryConfig>): UseBillUploadReturn {
  ...
  const saveFormData = useCallback(async (data: BillUploadFormData) => {
    ...
    try {
      ...
      setFormData(data);
      console.log('📝 [BILL UPLOAD] Form data saved to storage');
    } catch (err) {
  ...
  const loadFormData = useCallback(async (): Promise<BillUploadFormData | null> => {
    ...
    try {
      ...
      if (savedData) {
        ...
        setFormData(parsed);
        console.log('📂 [BILL UPLOAD] Form data loaded from storage');
        return parsed;
      }
    } catch (err) {
  ...
  const clearFormData = useCallback(async () => {
    ...
    try {
      ...
      setFormData(null);
      console.log('🗑️ [BILL UPLOAD] Form data cleared');
    } catch (err) {
  ...
  const handleProgress = useCallback((uploadProgress: UploadProgress) => {
    ...
    if (percent % 25 === 0 && percent > 0) {
      console.log(`📊 [BILL UPLOAD] ${percent}% complete - ${formatSpeed(uploadProgress.speed)}`);
    }
  }, []);
  ...
  const startUpload = useCallback(
    async (data: BillUploadData): Promise<boolean> => {
      try {
        console.log('🚀 [BILL UPLOAD] Starting upload...');
        ...
        if (result.success) {
          ...
          console.error('❌ [BILL UPLOAD] Upload failed:', result.error?.message);
        ...
        console.error('❌ [BILL UPLOAD] Exception during upload:', err);
  ...
  const retryUpload = useCallback(async (): Promise<boolean> => {
    if (!lastUploadDataRef.current) {
      console.error('❌ [BILL UPLOAD] No upload data available for retry');
      return false;
    }

    if (!canRetry) {
      console.error('❌ [BILL UPLOAD] Cannot retry - max attempts reached or error is not retryable');
      return false;
    }

    console.log(`🔄 [BILL UPLOAD] Retrying upload (attempt ${currentAttempt + 1}/${maxAttempts})...`);
  ...
  const cancelUpload = useCallback(() => {
    if (!isUploading || !uploadIdRef.current) {
      console.warn('⚠️ [BILL UPLOAD] No active upload to cancel');
      return;
    }

    console.log('🛑 [BILL UPLOAD] Cancelling upload...');
  ...
  const reset = useCallback(() => {
    console.log('🔄 [BILL UPLOAD] Resetting state...');
    setIsUploading(false);
    setUploadState('idle');
    setProgress(null);
    setError(null);
