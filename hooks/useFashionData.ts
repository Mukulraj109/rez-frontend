...
export const useFashionData = (): UseFashionDataResult => {
      ...
      if (response.success && response.data) {
        ...
        const fashionStoresData = Array.isArray(response.data) 
          ? response.data.filter((store: any) => {
              ...
              if (isFashion) {
                console.log('✅ Fashion Store:', store.name);
              }
        ...
        console.log(`📦 Found ${fashionStoresData.length} fashion stores`);
  ...
  const fetchFashionStores = useCallback(async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        console.log(`📦 Found ${fashionStoresData.length} fashion brands`);
  ...
  const fetchFeaturedProducts = useCallback(async () => {
    try {
      ...
      console.log('🔍 [FASHION DATA] Fetching featured products...');
      const response = await productsApi.getFeaturedProducts(20);
      
      console.log('📡 [FASHION DATA] API Response:', response);
      
      if (response.success && response.data) {
        console.log(`📦 [FASHION DATA] Total products received: ${response.data.length}`);
        
        // Log first product to see structure
        if (response.data.length > 0) {
          console.log('🔍 [FASHION DATA] Sample product:', {
        ...
        const fashionProductsData = Array.isArray(response.data)
          ? response.data.filter((product: any) => {
              ...
              if (isFashion) {
                console.log('✅ Fashion Product:', product.name);
              } else {
                console.log('❌ NOT Fashion:', product.name);
              }
        ...
        console.log(`📦 [FASHION DATA] Found ${fashionProductsData.length} fashion products out of ${response.data.length} total`);
        setFeaturedProducts(fashionProductsData.slice(0, 10));
      } else {
        console.log('⚠️ [FASHION DATA] No products data in response');
      }
    } catch (error) {
      console.error('❌ [FASHION DATA] Error fetching featured products:', error);
  ...
  const fetchCategories = useCallback(async () => {
    try {
      ...
      console.log('🔍 [FASHION DATA] Fetching all categories...');
      const response = await categoriesApi.getCategories();
      
      if (response.success && response.data) {
        console.log(`📦 [FASHION DATA] Received ${response.data.length} total categories`);
        ...
        console.log('🎨 [FASHION DATA] Main fashion category:', mainFashionCategory?.name, mainFashionCategory?._id);
        
        if (!mainFashionCategory) {
          console.log('⚠️ [FASHION DATA] Fashion & Beauty category not found!');
          setCategories([]);
          return;
        }
        ...
        const fashionCategories = Array.isArray(response.data)
          ? response.data.filter((category: any) => {
              ...
              if (isNonFashion) {
                console.log(`   ❌ Excluding non-fashion category: ${category.name} (${slug})`);
                return false;
              }
              
              if (isFashion) {
                console.log(`   ✅ Fashion Subcategory: ${category.name} (${slug})`);
              }
        ...
        console.log(`📦 [FASHION DATA] Found ${fashionCategories.length} fashion subcategories`);
        
        // Show up to 8 subcategories
        setCategories(fashionCategories.slice(0, 8));
      } else {
        console.log('⚠️ [FASHION DATA] No categories data in response');
        setCategories([]);
      }
    } catch (error) {
      console.error('❌ [FASHION DATA] Error fetching categories:', error);
