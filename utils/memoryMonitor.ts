...
class MemoryMonitor {
  ...
  async clearCaches(): Promise<void> {
    try {
      ...
      if (cacheKeys.length > 0) {
        ...
        console.log(`🧹 [MemoryMonitor] Cleared ${cacheKeys.length} cache entries`);
      }
    } catch (error) {
  ...
  forceGC() {
    if (typeof global.gc === 'function') {
      ...
      console.warn('⚠️ [MemoryMonitor] GC not available (run with --expose-gc flag)');
    }
  }
}
