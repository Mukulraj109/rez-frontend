...
class RequestDeduplicator {
  ...
  async deduplicate<T>(key: string, request: () => Promise<T>): Promise<T> {
    ...
    if (this.pendingRequests.has(key)) {
      console.log('🔄 [Deduplication] Using cached request:', key);
      return this.pendingRequests.get(key)!;
    }

    // Create new request
    console.log('🆕 [Deduplication] Creating new request:', key);
...
export class Lazy<T> {
  ...
  get(): T {
    if (!this.isEvaluated) {
      console.log('🔄 [Lazy] Evaluating lazy value');
...
export const measure = {
  ...
  async time<T>(label: string, fn: () => Promise<T>): Promise<T> {
    ...
    try {
      ...
      const duration = performance.now() - start;
      console.error(`❌ [Measure] ${label} failed after: ${duration.toFixed(2)}ms`);
      throw error;
    }
  },
