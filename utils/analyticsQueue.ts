...
export class AnalyticsQueue {
  ...
  async enqueue(event: AnalyticsEvent): Promise<void> {
    ...
    if (this.queue.length > MAX_QUEUE_SIZE) {
      console.warn('📊 [Queue] Max size reached, removing oldest events');
  ...
  private async processQueue(): Promise<void> {
    ...
    console.log(`📊 [Queue] Processing ${this.queue.length} events`);
    ...
    for (const event of eventsToProcess) {
      try {
        ...
        console.error('📊 [Queue] Failed to send event:', error);
        ...
        if (event.retryCount < MAX_RETRIES) {
          ...
          console.warn('📊 [Queue] Max retries reached, dropping event:', event);
        }
      }
    }
